...
const RoadmapSection: React.FC = () => {
  ...
  const quarters = t('quarters', { returnObjects: true }) as {
    label: string;
    status: string;
          ...
          <div className="space-y-8 lg:space-y-20">
            {quarters.map((quarter, index) => (
              ...
              >
                <div className={`${index % 2 === 0 ? 'lg:order-1' : 'lg:order-2'} lg:w-1/2`}>
                  <div className="glass rounded-2xl p-6 sm:p-8 border border-border/50">
                    <div className="flex items-center mb-6">
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center mr-4 ${getStatusColor(quarter.status)} bg-opacity-20`}
                      >
                    ...
                    <div className="space-y-4">
                      {quarter.features.map((feature) => (
                        <div
                          key={feature.title}
                          className="flex items-start p-4 bg-bg-tertiary/50 rounded-xl hover:bg-bg-tertiary transition-colors"
                        >
                          <div
                            className={`w-6 h-6 rounded-full flex items-center justify-center mr-4 mt-1 ${getStatusColor(quarter.status)} bg-opacity-20`}
                          >
                            {getStatusIcon(quarter.status)}
                          </div>
                          <div>
                ...
                <div
                  className={`${index % 2 === 0 ? 'lg:order-2' : 'lg:order-1'} lg:w-1/2 flex justify-center hidden lg:flex`}
                >
                  <div className="relative">
                    ...
                    <div
                      className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full ${getStatusColor(quarter.status)} opacity-20`}
                    ></div>
                  </div>
                </div>
              </motion.div>
