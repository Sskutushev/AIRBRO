...
const SolutionSection: React.FC = () => {
  ...
  const benefits = t('benefits', { returnObjects: true }) as {
    title: string;
    description: string;
    highlights: string[];
    icon: string;
  }[];
  const comparisonData = t('comparison.data', { returnObjects: true }) as {
    feature: string;
    freelancer: string;
    corp: string;
    aibro: string;
    ...
    <section className="py-20 bg-gradient-to-b from-bg-secondary to-bg-primary" id="solution">
      <div className="container mx-auto px-4">
            ...
            <table className="w-full border-collapse table-fixed">
              <thead>
                <tr className="bg-bg-tertiary">
                  <th className="p-4 text-left text-text-primary font-semibold w-[40%]">
                    {t('comparison.headers.feature')}
                  </th>
                  <th className="p-4 text-center text-amber-600 font-semibold w-[20%]">
                    {t('comparison.headers.freelancer')}
                  </th>
                  <th className="p-4 text-center text-blue-600 font-semibold w-[20%]">
                    {t('comparison.headers.corp')}
                  </th>
                  <th className="p-4 text-center text-primary-telegram font-semibold w-[20%]">
                    {t('comparison.headers.aibro')}
                  </th>
                </tr>
              </thead>
              <tbody>
                {comparisonData.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-center text-text-secondary break-words">
                      {row.freelancer}
                    </td>
                    <td className="p-4 text-center text-text-secondary break-words">{row.corp}</td>
                    <td className="p-4 text-center text-primary-telegram font-medium break-words">
                      {row.aibro}
                    </td>
                  </motion.tr>
                ))}
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center p-3 bg-gradient-to-r from-primary-telegram/20 to-primary-electric/20 rounded-lg">
                    <span className="text-primary-telegram font-medium">
                      {t('comparison.headers.aibro')}
                    </span>
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h3 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mb-3 sm:mb-4 text-[#00DDFD]">
            {t('cta.title')}
          </h3>
          <button
